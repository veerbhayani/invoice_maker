...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> saveDetailsToSharedPreferences() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();

    // Convert products list to List<Map<String, dynamic>> (JSON format)
    List<Map<String, dynamic>> productsJsonList =
        controller.productsList.map((product) => product.toJson()).toList();

    // Convert customers list to List<Map<String, dynamic>> (JSON format)
    List<Map<String, dynamic>> customersJsonList =
        controller.customersList.map((customer) => customer.toJson()).toList();

    // Save products list as a string
    prefs.setStringList(
        'productsList', productsJsonList.map((e) => json.encode(e)).toList());

    // Save customers list as a string
    prefs.setStringList(
        'customersList', customersJsonList.map((e) => json.encode(e)).toList());
  }

  // Function to load products and customer details from SharedPreferences
}
